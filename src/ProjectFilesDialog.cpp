...
ProjectFilesDialog::inProjectFiles() const
{
	...
	m_ptrInProjectFiles->items([&files](auto const& item) { pushFileInfo<Item>(files, item); });
...
ProjectFilesDialog::setInputDir(QString const& dir, bool const auto_add_files)
{
	...
	
	{
		...
		m_ptrInProjectFiles->files(
			[&existing_files](auto const& file) { existing_files.push_back(file); }
		);
...
ProjectFilesDialog::addToProject()
{
	...
	m_ptrOffProjectFiles->items(selection, [&items](auto const& item) { items.push_back(item); });
...
ProjectFilesDialog::removeFromProject()
{
	...
	m_ptrInProjectFiles->items(
		selection, [&items, &input_dir](auto const& item) {
...
ProjectFilesDialog::FileList::remove(QItemSelection const& selection)
{
	...
	std::transform(
		...
		[](auto const& range) { return Range(range.top(), range.bottom()); }
	);
	...
	std::sort(
		...
		[](auto const& r1, auto const& r2) { return r1.first < r2.first; }
	);
...
ProjectFilesDialog::FileList::prepareForLoadingFiles()
{
	...
	std::sort(
		...
		[this](int idx1, int idx2) { return ItemVisualOrdering()(m_items[idx1], m_items[idx2]); }
	);
...
ProjectFilesDialog::FileList::loadNextFile()
{
	...
	ImageMetadataLoader::Status const st = ImageMetadataLoader::load(
		file_path, [&per_page_metadata](auto const& metadata) {
